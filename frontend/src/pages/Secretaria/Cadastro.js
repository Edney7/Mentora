// pages/Secretaria/Cadastro.js (Completo com queryFn corrigido)
...
import { buscarTurmasAtivas, buscarDisciplinas, cadastrarUsuario } from "../../services/ApiService";
...
export default function Cadastro() {
  ...
  const form = useForm({
    defaultValues: {
      ...
      tipoUsuario: "", sexo: "", dtNascimento: "", turmaId: "", disciplinaIds: [],
    },
  });
  ...
  const { data: turmas = [], isLoading: isTurmasLoading } = useQuery({
    ...
    queryFn: buscarTurmasAtivas, // Propriedade restaurada
  ...
  const { data: disciplinas = [], isLoading: isDisciplinasLoading } = useQuery({
    ...
    queryFn: buscarDisciplinas, // Propriedade restaurada
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input placeholder="000.000.000-00" {...field} onChange={(e) => field.onChange(formatarCPF(e.target.value))} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                ...
                {tipoUsuario === "ALUNO" && (
                   <FormField control={form.control} name="turmaId" render={({ field }) => (
                      <FormItem>
                        ...
                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                          <FormControl><SelectTrigger><SelectValue placeholder={isTurmasLoading ? "Carregando turmas..." : "Selecione a turma"}/></SelectTrigger></FormControl>
                          <SelectContent>
                            {!isTurmasLoading && turmas.map((turma) => (<SelectItem key={turma.id} value={turma.id.toString()}>{turma.nome}</SelectItem>))}
                          </SelectContent>
                        </Select>
                        <FormMessage />
                      </FormItem>
                ...
                {tipoUsuario === "PROFESSOR" && (
                  <FormField control={form.control} name="disciplinaIds" render={() => (
                      <FormItem>
                        <FormLabel>Disciplinas</FormLabel>
                        <div className="p-2 border rounded-md max-h-40 overflow-y-auto">
                          {isDisciplinasLoading ? <p className="text-sm text-gray-500">Carregando...</p> : disciplinas.map((item) => (
                            <FormField key={item.id} control={form.control} name="disciplinaIds" render={({ field }) => (
                                  <FormItem key={item.id} className="flex flex-row items-start space-x-3 space-y-0">
                                    ...
                                    <FormLabel className="font-normal">{item.nome}</FormLabel>
                                  </FormItem>
                                )}/>
                          ))}
                        </div>
              ...
              <div className="flex justify-end pt-4">
                <Button type="submit" disabled={cadastroMutation.isPending} className="w-full md:w-auto">
                  {cadastroMutation.isPending ? "Cadastrando..." : "Cadastrar Usuário"}
                </Button>
              </div>
            </form>
          </Form>
