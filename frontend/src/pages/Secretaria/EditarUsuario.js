...
import {
  buscarUsuarioPorIdIncluindoInativos,
  atualizarUsuario,
...
// Converte dd-MM-yyyy (backend) para yyyy-MM-dd (input)
...
export default function EditarUsuario() {
  ...
  const {
    ...
    queryKey: ["usuario", id],
    queryFn: () => buscarUsuarioPorIdIncluindoInativos(id),
  });
  ...
  useEffect(() => {
    if (usuario) {
      reset({
        ...
        email: usuario.email || "",
        sexo: usuario.sexo || "",
        dtNascimento: formatarDataParaInput(usuario.dtNascimento), // Apenas para exibição
      });
    }
  }, [usuario, reset]);
  ...
  function onSubmit(data) {
    ...
    // AQUI ESTÁ A CORREÇÃO: Removemos a data de nascimento do objeto
    delete dataToSend.dtNascimento;
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>CPF (não pode ser alterado)</FormLabel>
                      <FormControl>
