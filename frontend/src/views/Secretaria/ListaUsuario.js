...
import { 
    listarTodosOsUsuarios, // Para a secretaria ver todos
    desativarUsuario, 
    reativarUsuario 
...
export default function ListaUsuario() {
  ...
  const carregarUsuarios = useCallback(async () => {
    ...
    try {
      ...
      const data = await listarTodosOsUsuarios(filtros); // Secretaria vê todos
      setUsuarios(data || []);
    } catch (error) {
  ...
  const usuariosFiltrados = usuarios.filter((usuario) => {
    ...
    const nomeMatch = usuario.nome?.toLowerCase().includes(nomeFiltro.toLowerCase());
        ...
        <div className="usuarios-header">
          ...
          <div className="usuarios-filtros">
            ...
            <select value={sexoFiltro} onChange={(e) => setSexoFiltro(e.target.value)}>
              ...
              <option value="Masculino">Masculino</option> {/* Certifique-se que estes valores correspondem aos do seu DTO/backend */}
            ...
            <button onClick={() => navigate("/cadastro")} className="btn-add" title="Cadastrar Novo Usuário">
              <FaPlus/>
            </button>
          </div>
        </div>
              ...
              <div className="usuario-row" key={usuario.id}>
                ...
                <div className="usuario-conteudo">
                  <div className="usuario-info">
                    ...
                    <span><strong>Nasc:</strong> {usuario.dtNascimento}</span> {/* Formato da data do DTO */}
                    <span><strong>Status:</strong> <span className={usuario.ativo ? 'status-ativo' : 'status-inativo'}>{usuario.ativo ? "Ativo" : "Inativo"}</span></span>
                  </div>
                  <div className="usuario-acoes">
                    <button 
                        onClick={() => alert(`Editar usuário ID: ${usuario.id}. Funcionalidade a implementar.`)} 
                    ...
                    {usuario.ativo ? (
                      <button onClick={() => handleDesativar(usuario.id, usuario.nome)} title="Desativar Usuário">
                        <FaTrash />
                      </button>
                    ) : (
                      <button onClick={() => handleReativar(usuario.id, usuario.nome)} title="Reativar Usuário" className="btn-action btn-reactivate"> {/* Adicione a classe btn-action se quiser o mesmo estilo dos outros */}
                        <FaRedo /> 
                      </button>
                    )}
                  </div>
